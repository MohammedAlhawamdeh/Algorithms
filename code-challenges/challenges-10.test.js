...
const count = (target, input) => {
  const concated = input.reduce((acc, val) => {
    acc = acc.concat(val)
    return acc
  }, [])
  const counted = concated.reduce((accu, value) => {
    if (target === value) {
      accu += 1
    }
    return accu
...
const totalSum = (input) => {
  ...
  const sum = concated.reduce((accu, value) => {
    accu += value
    return accu
  }, 0)
  return sum
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((big) => {
    const valued = big.reduce((acc, val) => {
...
let findMaleAndFemale = (data) => {
  const have = data.reduce((acc, val) => {
    if (val.gender === 'female' || val.gender === 'male') {
      acc.push(val.name)
    }
    return acc
  }, [])
  return have.join(' and ')
};
