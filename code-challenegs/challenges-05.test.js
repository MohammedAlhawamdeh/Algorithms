...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= str.length; i++) {
    let edited = str.slice(i, str.length)
    result.push(edited)
  }
  return result;
};
...
const listFoods = (recipe) => {
  let cook = recipe.ingredients
  let result = [];
  for (let i = 0; i < cook.length; i++) {
    let firstSpace = cook[i].slice(cook[i].indexOf(' ') + 1)
    let secondSpace = firstSpace.slice(firstSpace.indexOf(' ') + 1)
    result.push(secondSpace)
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  let result = [];
  for (let i = 0; i < cook.length; i++) {
    let splitArray = cook[i].split(' ')
    result.push(splitArray.splice(2).join(' '))
  }
  return result;
};
/* ------------------------------------------------------------------------------------------------
...
const stepActions = (recipe) => {
  let cook = recipe.steps
  let result = [];
  for (let i = 0; i < cook.length; i++) {
    let splitted = cook[i].split(' ')
    let sliced = splitted.slice(0, 1)
...
const totalSumCSV = (str) => {
  ...
  for (let i = 0; i < splitted.length; i++) {
    if (parseInt(splitted[i]) > 0) {
      total = total + parseInt(splitted[i])
    }
  }
  return total;
};
...
const extractVowels = (str) => {
  ...
  for (let i = 0; i < splitted.length; i++) {
    if (vowels.includes(splitted[i])) {
      let spliced = splitted.splice(i--, 1)
      vowelsLetters.push(spliced.join(''))
    }
  }
  return [splitted.join(''), vowelsLetters.sort().join('')]
